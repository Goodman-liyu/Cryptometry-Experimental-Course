...
class MainWindow:
    def __init__(self, master):
        ...
        self.key_size_label.grid(row=0, column=0, padx=100, pady=40, sticky=tk.W)
        # 生成密钥按钮
        self.open_next_window_button = tk.Button(master, text="生成密钥", width=10, height=2,
                                                 command=self.open_next_window,
                                                 bg="#{:02x}{:02x}{:02x}".format(165, 244, 228),
                                                 fg="#{:02x}{:02x}{:02x}".format(42, 87, 228),
                                                 font=("Helvetica", 12, "bold"))
        self.open_next_window_button.grid(row=1, column=0, ipadx=5, ipady=5, padx=80, pady=40)

        # RSA加密按钮
        self.open_next_window_button2 = tk.Button(master, text="RSA加密", width=10, height=2,
                                                  command=self.open_next_window2,
                                                  bg="#{:02x}{:02x}{:02x}".format(165, 244, 228),
                                                  fg="#{:02x}{:02x}{:02x}".format(42, 87, 228),
                                                  font=("Helvetica", 12, "bold"))
        self.open_next_window_button2.grid(row=1, column=1, ipadx=5, ipady=5, padx=0, pady=0)

        # RSA解密按钮
        self.open_next_window_button3 = tk.Button(master, text="RSA解密", width=10, height=2,
                                                  command=self.open_next_window3,
                                                  bg="#{:02x}{:02x}{:02x}".format(165, 244, 228),
                                                  fg="#{:02x}{:02x}{:02x}".format(42, 87, 228),
                                                  font=("Helvetica", 12, "bold"))
        self.open_next_window_button3.grid(row=2, column=0, ipadx=5, ipady=5, padx=0, pady=0)

        # 退出按钮
        self.exit_button = tk.Button(master, text="退出", width=10, height=2, command=self.on_exit,
                                     bg="#{:02x}{:02x}{:02x}".format(165, 244, 228),
                                     fg="#{:02x}{:02x}{:02x}".format(42, 87, 228), font=("Helvetica", 12, "bold"))
        self.exit_button.grid(row=2, column=1, ipadx=5, ipady=5, padx=0, pady=0)
...
class Next_generate:
    def __init__(self, master, name):
        ...
        self.key_size_combobox = ttk.Combobox(self.frame, values=[64, 128, 256, 512, 1024], state="readonly",
                                              font=("Helvetica", 12))
...
class Next_encryption:
    def __init__(self, master, name):
        ...
        self.file_path_entry.grid(row=0, column=0, ipady=3)
        # 浏览文件按钮
        self.browse_button = ttk.Button(self.frame, text="选择需要加密的文件", command=self.browse_file,
                                        style="TButton")
    ...
    def encrypt(self):
        ...
        if e is not None and n is not None:
            self.encode_str = encode(self.file_path, e, n)
...
class Next_decryption:
    def __init__(self, master, name):
        ...
        # 文件路径输入框
        self.file_path_entry = ttk.Entry(self.frame, width=50, state='disabled', font=("Helvetica", 12))
        self.file_path_entry.grid(row=0, column=0, padx=10, pady=10, ipady=3)
        # 浏览文件按钮
        self.browse_button = ttk.Button(self.frame, text="选择需要解密的文件", command=self.browse_file,
                                        style="TButton")
        self.browse_button.grid(row=0, column=1, padx=10, pady=10)
        ...
        self.back_to_main_button = ttk.Button(self.frame, text="返回主窗口", command=self.back_to_main, style="TButton")
        self.back_to_main_button.grid(row=8, column=1, padx=0, pady=0)
    ...
    def decrypt(self):
        d = simpledialog.askinteger("输入", "私钥d:", parent=self.master)
        n = simpledialog.askinteger("输入", "模n:", parent=self.master)

        if d is not None and n is not None:
            self.decode_str = decode(self.file_path, d, n)
